...
function daftar($post)
{
    ...
    $infotali = htmlspecialchars($post['infotali']);
    ...
    if (empty($kelamin)) {
        $_SESSION['pesan'] = 'kelamin harus diisi!';
        return;
    }  
    if (empty($infotali)) {
        $_SESSION['pesan'] = 'info talikur harus diisi!';
        return;
    }        
    if (empty($password1)) {
    ...
    mysqli_query($konek, "INSERT INTO users VALUES ('','$nama','$email','$alamat','$kelamin','$infotali','$sandi','$image', '2', '$createat', '') ");
